...
- Git-changed files (and the directories containing them) are coloured
  ...
  committed") in green, modified-but-unstaged and untracked paths in red. This
  works whether the tree root is itself a repository or just a parent folder
  holding several separate checkouts — each directory with its own `.git` is
  coloured from its own status. The colouring refreshes as sessions edit files.
