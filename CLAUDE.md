...
- `select.rs` — mouse text selection over the terminal pane. With the host terminal in mouse-capture mode the OS can't do its own drag-select, so we do it: `run.rs` tracks a `Selection { anchor, cursor }` of pane-relative cells across left-button drag (`begin_selection`/`update_selection` on `App`), `ui.rs::paint_selection` flips `REVERSED` on the covered cells, and on release `select::selected_text` pulls the text out of the vt100 screen via `Screen::contents_between` (the canonical clipboard helper — trims trailing blanks, stitches wrapped rows). The selection clears on any keystroke/scroll/focus-change that disturbs the screen underneath it.
- `clipboard.rs` — copy to the system clipboard, used by the selection path. Two best-effort routes together: an **OSC 52** escape written to our own stdout (we run *outside* tmux, so it reaches the host terminal directly; portable + SSH-friendly), plus a **local clipboard tool** (`pbcopy` / `wl-copy` / `xclip` / `xsel`) for terminals that ignore OSC 52. Includes a tiny dependency-free base64 encoder for the OSC 52 payload.
