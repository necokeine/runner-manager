...
- `tmux.rs` — all tmux interaction goes through the `CommandRunner` trait (`SystemRunner` in prod, `MockRunner` in tests). `Tmux<R>` prefixes every call with `-S <socket-path>`. Everything downstream is generic over `R: CommandRunner`, which is what makes `App` unit-testable without a real tmux. Sessions created from Codex history also get `@rm_resume=<id>` so the project view can mark the matching history entry as running and switch to it instead of opening duplicates.
...
- `resume.rs` — shared `ResumeSession { id, last_command, modified }` returned by agent-history discovery modules and rendered by the chooser.
- `claude.rs` — discovery of resumable Claude Code sessions. Claude keeps one JSONL transcript per session under `~/.claude/projects/<encoded cwd>/<uuid>.jsonl`; those files *are* the record of resume ids, so there's nothing extra to persist. `encode_project_dir()` reproduces Claude's folder naming (every non-ASCII-alphanumeric byte → `-`), `projects_base()` resolves `~/.claude/projects` (overridable via `RM_CLAUDE_PROJECTS`), and `list_sessions(base, dir)` returns the newest few transcripts, parsing each transcript's last genuine user prompt.
- `codex.rs` — discovery of resumable Codex CLI sessions. Codex keeps JSONL transcripts under `~/.codex/sessions/YYYY/MM/DD/*.jsonl`, so `sessions_base()` resolves that root (overridable via `RM_CODEX_SESSIONS`, or `CODEX_HOME/sessions`), scans recent transcript files, filters by `session_meta.payload.cwd`, and returns the newest matching `ResumeSession`s. `list_sessions_under(base, root)` powers the project view's Codex history group. Creating from a selected Codex resume launches `codex resume <id>`.
...
- `rows.rs` — flattens the tree + per-dir sessions into a single `Vec<Row>` (the visible list). `RowKind` is `Header | Dir | Session | CodexResume | File`; `selected` indexes into this vec. The project view emits `Live sessions` and `Codex history` headers, then marks Codex history entries as `running` when their resume id appears in the live `@rm_resume` map. **Rebuild rows (`App::rebuild_rows`) after any tree expand/collapse or session change** — the row vec is derived state.
- `app.rs` — `App<R>` holds all state and the action methods (`activate`, `open_chooser`, chooser state machine, `switch_to`, `sync`, split sizing). `restore_expanded()` (called once at startup) re-expands the saved dirs; `persist_expanded()` is called after every dir toggle in `activate` to save the new state. No I/O event handling here. The chooser (`Popup::Chooser`) is a small radio-form state machine: `chooser_rows()` derives the visible focusable rows (the claude permission rows only appear when kind == Claude; a `Resume` group — `ResumeNew` + one `Resume(i)` per discovered session — appears when the selected agent has local history), and selection follows focus. `open_chooser` populates Claude and Codex resume lists for the chosen dir, and `chooser_command(kind, perm, resume_id)` builds the launch command per kind: shell → none (default shell), claude → `claude [--resume <id>] [--dangerously-skip-permissions]`, codex → `codex [resume <id>]`.
