...
- `codex.rs` — discovery of resumable Codex CLI sessions. Codex keeps JSONL transcripts under `~/.codex/sessions/YYYY/MM/DD/*.jsonl`, so `sessions_base()` resolves that root (overridable via `RM_CODEX_SESSIONS`, or `CODEX_HOME/sessions`), scans recent transcript files, filters by `session_meta.payload.cwd`, and returns the newest matching `ResumeSession`s. Creating from a selected Codex resume launches `codex resume <id>`.
