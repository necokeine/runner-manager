...
- `term/` — the embedded terminal: the PTY running the tmux client (`pty.rs`) and keystroke encoding (`keys.rs`).
...
- `run.rs` — the composition root: real-terminal setup/teardown and the event loop.
...
- `project/git.rs` — the `git status` scan (`GitStatuses::load`) that colours tree rows: parses porcelain `-z` output per repo (the root's containing repo plus nested repos), staged → green, dirty/untracked → red, ignored → grey. Always run on a worker thread (see `run.rs`).
...
- `term/pty.rs` / `term/keys.rs` — the embedded terminal: one PTY running the tmux client with a reader thread feeding a shared vt100 parser (dropping the `Pty` kills the child), and the crossterm→bytes key/wheel encoding the run loop writes to it.
