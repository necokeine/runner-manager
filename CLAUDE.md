...
- `tmux.rs` — all tmux interaction goes through the `CommandRunner` trait (`SystemRunner` in prod, `MockRunner` in tests). `Tmux<R>` prefixes every call with `-L <socket>`. Everything downstream is generic over `R: CommandRunner`, which is what makes `App` unit-testable without a real tmux.
