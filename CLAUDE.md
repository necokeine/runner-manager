...
- `project/tree.rs` — lazy filesystem tree; every expand re-reads the dir from disk (`Node::refresh_children`, called by `toggle`) so the listing is never stale, preserving the expanded state and loaded subtrees of surviving subdirectories (expanded survivors refresh recursively). Dirs sorted before files. `expanded_dirs()` collects the visible expanded subtree (for persistence) and `apply_expanded()` re-expands a saved set shallow-to-deep, loading children lazily.
